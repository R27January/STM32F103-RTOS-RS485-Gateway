...
#define PROTOCOL_HEAD2              0x55  /* 帧头�?字节，固定为0x55 */

/* Protocol command */
#define CMD_GET_SENSOR              0x01  /* 主机查询传感器数据的请求命令 */
#define CMD_RESP_SENSOR             0x81  /* 设备返回传感器数据的响应命令 */
#define CMD_RESP_ERROR              0xE0  /* 设备返回协议错误的响应命�?*/

#define CMD_GET_LOG                 0x02
#define CMD_RESP_LOG                0x82
/* Protocol LEN field: length of CMD + DATA */
#define PROTOCOL_LEN_GET_SENSOR     0x01  /* 请求帧LEN字段：CMD+DATA�?字节，只有CMD */
#define PROTOCOL_LEN_RESP_SENSOR    0x05  /* 正常响应LEN字段：CMD+DATA�?字节 */
#define PROTOCOL_LEN_RESP_ERROR     0x02  /* 错误响应LEN字段：CMD+DATA�?字节 */
#define PROTOCOL_LEN_RESP_LOG       0x0B
#define PROTOCOL_CRC_LEN            2     /* CRC16字段长度：CRC_L和CRC_H�?字节 */
#define PROTOCOL_LEN_GET_LOG        0x05
#define PROTOCOL_LEN_GET_DEVICE_INFO    0x01

/* Whole frame length */
#define FRAME_LEN_GET_SENSOR        6   /* 请求整帧长度：帧�?+LEN1+CMD1+CRC2 */
#define FRAME_LEN_RESP_SENSOR       10    /* 正常响应整帧长度：帧�?+LEN1+CMD/DATA5+CRC2 */
#define FRAME_LEN_RESP_ERROR        7     /* 错误响应整帧长度：帧�?+LEN1+CMD/DATA2+CRC2 */
...
#define ERR_CODE_LOG_INVALID        0x05  
            ...
            if(protocol_ret == PROTOCOL_OK)
            {
              ...
              else if (rx_buf[3] == CMD_GET_LOG)
              {
                ...
                query_seq = ((uint32_t)rx_buf[4] << 24) |
                            ...
                            ((uint32_t)rx_buf[6] << 8)  |
                            ((uint32_t)rx_buf[7]);
                if (query_seq >= LOG_MAX_COUNT)
                {
                  Protocol_SendError(ERR_CODE_LOG_INVALID);
                }
                else
                {
              ...
              else if (rx_buf[3] == CMD_CLEAR_LOG)
              {
                if (rx_buf[4] == 'C' && rx_buf[5] == 'L' && rx_buf[6] == 'R' && rx_buf[7] == '!' )
                {
                  ...
                  Protocol_SendClearLogResponse(0x00);
                }
                else
                {
                  Protocol_SendClearLogResponse(0x01);
                }
              }
...
{ 
  ...
  {
    if (rx_buf[2] != PROTOCOL_LEN_CLEAR_LOG)
    {
      return ERR_CODE_LEN;
    }
...
static void CommTask_SendQueryOkLog(SensorData_t sensor)
{
  ...
  log.err_code = 0;
  log.source = LOG_SOURCE_LOCAL;
  log.target_id = LOG_TARGET_NONE;
